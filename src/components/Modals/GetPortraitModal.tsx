...
export default function GetPortraitModal(props: GetPortraitModalProps) {
	...
	// Função que atualiza a cor e salva no banco e no localStorage
	function handleColorChange(newColor: string) {
		setDiceColor(newColor);
		const cleanColor = newColor.substring(1); // Remove o '#'
					...
					<Row className='mb-3'>
						...
						<Col xs={2}>
							<FormControl
								...
								type='color'
								value={diceColor}
								onChange={(ev) => handleColorChange(ev.currentTarget.value)}
